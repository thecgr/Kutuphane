...
{
    public partial class BagisForm : MetroForm
    {
        ...
        {
            cmbKitapTuru.DataSource = Enum.GetValues(typeof(KitapTurEnum));
        }

        private void btnBagisYap_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtKitapAdi.Text) && !string.IsNullOrEmpty(txtYazarAd.Text))
            {
                kutuphaneYoneticisi.KitapBagisYap(
                    txtKitapAdi.Text,
                    dtpBasimTarihi.Value,
                    (KitapTurEnum)cmbKitapTuru.SelectedItem,
                    txtYazarAd.Text,
